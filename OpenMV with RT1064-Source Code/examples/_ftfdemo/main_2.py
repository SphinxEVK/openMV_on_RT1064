...
barCol = 0
def DrawPgsBar(img, barLen, loopCnt, startTick, sTitle, width=5):
    global barCol
	...
	img.draw_string(4, 8, sTitle, color=(c,c,c))
    barCol += 16
...
def BlobTest(thresholds, loopCnt = 390, barLen = 120):
    ...
    while(True):
        ...
        lnLen = (barLen * (loopCnt - (time.ticks() - startTick))) // loopCnt
        DrawPgsBar(img, barLen, loopCnt, startTick,'red,green,blue blob detect')
        for blob in blobSet:
...
def CorrTest(loopCnt = 220, barLen=120):
    ...
    while (True):
        ...
        lnLen = (barLen * (loopCnt - (time.ticks() - startTick))) // loopCnt
        DrawPgsBar(img, barLen, loopCnt, startTick,'Lens correction %.2f' % (corr))
...
def FaceTest(loopCnt = 220, barLen=120):
    ...
    while (True):
        ...
        fID = 0
        lnLen = (barLen * (loopCnt - (time.ticks() - startTick))) // loopCnt
        DrawPgsBar(img, barLen, loopCnt, startTick, 'Face Detect')
        for r in objects:
...
def CIFAR10Test(loopCnt = 600, isFull = False, barLen = 105):
    ...
    while (True):
        ...
        lnLen = (barLen * (loopCnt - (time.ticks() - startTick))) // loopCnt
        DrawPgsBar(img, barLen, loopCnt, startTick, 'CIFAR-10: classify:\nplane,auto,cat,dog,\ndeer,horse,frog,ship,\ntruck,horse')
        for obj in lst:
